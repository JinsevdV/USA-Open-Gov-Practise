...
const app = new Vue({
  ...
  async mounted() {
    ...
    this.getVotes();
    this.getUpperAndLower('missed_votes_pct');
    this.getUpperAndLower('votes_with_party_pct');
  },
  methods: {
    sort: function (s) { // sorts A-Z Z-A
      if (s === this.currentSort) {
        this.currentSortDir = this.currentSortDir === 'asc' ? 'desc' : 'asc';
      }
      this.currentSort = s;
    },
    sortTop: function (s) { // added for 2nd table filter
      if (s === this.topSort) {
        this.topSortDir = this.topSortDir === 'desc' ? 'asc' : 'desc';
      }
      this.topSort = s;
    },
    sortBottom: function (s) { // added for 2nd table filter
      if (s === this.bottomSort) {
        this.bottomSortDir = this.bottomSortDir === 'asc' ? 'desc' : 'asc';
      }
      this.bottomSort = s;
    },
    filter: function (column, value) { //sorts Rep/Dem/Ind
      let contains = false;
    ...
    setState: function (event) { //sorts state
      this.state = event.target.value;
    },
    searchName: function (event) { //sorts searchbar
      this.name = event.target.value.toLowerCase();
    },
    getVotes: function () { //sorts num of reps and average value of party votes
      let repTotal = 0;
      let demTotal = 0;
      let indTotal = 0;
  ...
  computed: {
    ...
    topTenSorted: (app) => (column) => {
      if (app.topTen[column] === undefined) return [];
    ...
    bottomTenSorted: (app) => (column) => { //adds a sort for bottom table
      if (app.bottomTen[column] === undefined) return [];
